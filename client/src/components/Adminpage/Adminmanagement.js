...
const Adminmanagement = () => {
  ...
  const searchUserAndRole = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:3001/api/user/${searchQuery}`);
  ...
  return (
    <div className="container mt-5">
      ...
      {role && (
        <div className="row justify-content-center mt-3">
          <div className="col-md-6 text-center">
            <p>User role: <strong>{role}</strong></p>
            {role !== 'admin' && (
              <button onClick={promoteToAdmin} className="btn btn-success">Promote to Admin</button>
            )}
          </div>
        </div>
      )}
      {message && (
        <div className="row justify-content-center mt-3">
          <div className="col-md-6">
            <div className={`alert ${role ? 'alert-success' : 'alert-danger'}`} role="alert">
              {message}
            </div>
          </div>
        </div>
